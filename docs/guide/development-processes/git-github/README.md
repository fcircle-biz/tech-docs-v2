...
Git・VS Code・GitHubアカウントを準備し、`git config` で初期設定を行います。さらにSSH鍵を作成してGitHubに公開鍵を登録し、SSH方式で接続する準備まで整えます。
...
GitHubにリポジトリを作成し、SSH方式でローカルと接続して `push`・`clone`・`pull` でやり取りします。SSH接続でよくあるエラーの概要も押さえます。
...
- SSH鍵を作成・登録し、GitHubとSSH方式で安全に接続できる
