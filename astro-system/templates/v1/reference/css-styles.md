# CSS スタイルガイド v1（Graphite × Iris）

## 概要

v1 テンプレートは「**Graphite × Iris**」デザインシステムを採用しています。
...
HTML の本文クラス（`from-amber-50`、`border-l-4` など）は従来どおりのままで、
...
技術色は **データ.ts の `TechGuide.primary`（50〜900）1箇所**で指定する。`GuideChapterLayout.astro` が `primary` から以下の CSS 変数を導出して各章にインライン注入する（手で `styles.css` を編集しない・プレースホルダー置換は廃止）。

```css
/* レイアウトが primary から導出してインライン注入する :root 変数 */
--primary-300/400/500/600/700;        /* primary[300..700] */
--primary-rgb;                        /* primary[500] の RGB（例: 99, 102, 241） */
```

ダークモードでは自動的に 1段明るいシェードへ（レイアウトが注入）：

```css
html.dark { --primary-500: <primary[400]>; }
```
...
v1 では `fontFamily` に `mono` を追加し、Google Fonts も更新します。
...
## 3. 技術別テーマの設定方法（データ1箇所）

技術色は **データ.ts の `TechGuide.primary`（50〜900）1箇所**で指定する。`GuideChapterLayout.astro` がこの `primary` から `tailwind.config` の `primary` パレットと `:root` の CSS 変数（`--primary-300/400/500/600/700`・`--primary-rgb`、ダークの `--primary-500`）を導出してインライン注入する。

```ts
// 例: src/data/guides/<分類>/<slug>.ts（アイリス既定の場合）
primary: {
...
レイアウトが導出する内容:
- `--primary-rgb` = `primary[500]` の RGB（例: `#6366f1` → `99, 102, 241`）
- ダークの `--primary-500` = `primary[400]`（1段明るいシェード）

具体値は `reference/color-themes.md` を参照。共有 `_shared/styles.css` は技術色非依存（プレースホルダー置換・per-tech styles.css は廃止）。

---

## 4. Google Fonts リンク（v1 追加）

v1 では JetBrains Mono を追加しています。Noto Sans JP の weight も 800 まで拡張。
...
v1 のヘッダーは両モード共通でグラファイトのすりガラスデザインになります。
HTML クラスは 従来どおり `header-rich header-rich-shadow` のままです。
...
v1 のカードは「**ニュートラル面 + 色ヘアライン + グラデーションアイコンチップ**」が基本です。
...
v1 のコードブロックは JetBrains Mono フォントと macOS 風トラフィックドット付きウィンドウデザインです。
...
v1 では `border-l-4` の左罫をグラデーションバーに変換します。HTML はそのまま。
...
HTML クラスは 従来どおり。`styles.css` が自動適用します。
...
HTML クラスは 従来どおり。`styles.css` が自動で角丸・影・ヘッダー色を適用します。
...
Tailwind CSS の標準ブレークポイント（標準のまま）：
...
- `reference/color-themes.md` — 技術別カラーテーマの詳細（`primary` の具体値）
