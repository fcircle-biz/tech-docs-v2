...
description: 学習ガイドの章HTMLの内容を読み取って挿絵の図案を設計し、codex CLI の imagegen（GPT Image 2）で背景透過PNGを生成して work/<slug>/ に保存し、そのまま章HTMLへ <figure> 差し込み・ビルドまで行うスキル。教材に新しく挿絵イラスト／インフォグラフィックを作って入れたい際に使用する。
---
...
画像は**背景透過（アルファ付き PNG）**で出力する。GPT Image 2 は透過出力に非対応なので、**クロマキー背景で生成 → ローカルでアルファ化**する（`references/codex-imagegen.md`「透過PNG化」）。
...
5. **透過前提で構成を決める**。タイトルを含むすべての文字・細線を**不透明なカード／帯の中**に置く設計にする（地に直接置かない）。技術色がグリーン系ならキーカラーを `#ff00ff` にする。
   - 設計指針・重複棚卸しの判定表・命名規則・禁止事項・透過規約は `references/illustration-design.md` を参照。

### Step 3: codex で画像生成（クロマキー背景）

1. `work/<slug>/_raw/` を作成する（クロマキー原版の置き場）。
2. Step 2 の設計から**codex 用プロンプト**（Markdown）をスクラッチパッドに書き出す。背景の要件（フラットなキーカラー単色・影/グラデーション禁止・キーカラーを図版内で使わない）を必ず含める。
3. `codex exec` を**バックグラウンドで**実行し、`work/<slug>/_raw/` へ PNG を生成させる。**アルファ化は codex にさせない**（こちらで行う）。
4. 生成完了を待つ（Monitor で「codex プロセス終了」を待機）。
   - コマンド・プロンプトテンプレート・リトライ方針は `references/codex-imagegen.md` を参照。

### Step 4: 透過化と生成物の検証（**Opus＝本体が目視**）

1. `remove_chroma_key.py` を `uv run --with pillow` 経由で実行し、`work/<slug>/_raw/*.png` → `work/<slug>/*.png`（アルファ付き）へ変換する（`references/codex-imagegen.md`「変換コマンド」）。
2. `file work/<slug>/*.png` で寸法（既定 **1536x1024**）を、透過検証コマンドで **RGBA・四隅 alpha 0・透明率 10〜60%** を確認する。
3. **各画像を Read で開いて自分の目で確認する**（日本語の誤字・意味不明な綴り・レイアウト崩れ・ロゴ混入・配色ずれ・キーカラーのフリンジ）。
4. ダーク合成 `_check-dark-*.png` を作って**目視**し、ダークモードで消える文字・線が無いことを確認する。
5. 不合格なら Step 3 のプロンプトを修正して**再生成**する（同一章で最大2回まで。3回目で通らなければ所見をユーザーに報告して判断を仰ぐ）。
   - 合否基準は `references/illustration-design.md` の「検収チェック」を参照。

### Step 5: 差し込み指示Markdownの作成と差し込み・ビルド

1. `work/<slug>/差し込み箇所.md` を書き出す（`references/handoff-and-insert.md` の書式）。差し込み対象は **`work/<slug>/` 直下の透過PNG**であり、`_raw/` と `_check-dark-*.png` は対象外。
...
| 対象 | 規則 | 例 |
|------|------|-----|
| work/ 内のクロマキー原版（中間物） | `_raw/<slug>-<NN>-<topic>.png` | `_raw/markdown-01-overview.png` |
...
- **codex には画像生成以外をさせない**。プロンプトに「リポジトリの他ファイル（`src/`・`docs/`・`astro-system/`）は一切変更しない。作業は `work/<slug>/_raw/` への画像保存のみ」を必ず明記する。**透過化・差し込み・ビルドは本スキル側（Claude）が行う**（codex に `gpt-image-1.5` / `OPENAI_API_KEY` を使わせない）。
- **成果物は背景透過 PNG**。`_raw/`（クロマキー原版）と `_check-dark-*.png`（ダーク確認用）は中間物であり、`public/` へコピーしない。
...
| `references/codex-imagegen.md` | codex CLI の呼び出し方（`codex exec` コマンド・バックグラウンド実行・完了待ち）／**透過PNG化（クロマキー生成→アルファ化→検証）**／プロンプトテンプレート／スタイル参照画像の選定／リトライとトラブルシュート |
