...
画像は**背景透過（アルファ付き PNG）**で出力する。**組み込み `image_gen` ツールは真の透過に対応している**ので、プロンプトで「背景を完全に透明にし、アルファをそのまま保持する」と指示して**直接透過生成する**（`references/codex-imagegen.md`「方式① 直接透過生成」）。

> 「GPT Image 2 は透過非対応」という制約は **CLI フォールバック**（`scripts/image_gen.py`）の話であり、組み込みツールには当てはまらない。クロマキー生成＋ローカルでアルファ化は、直接透過に2回失敗したときの**フォールバック**（方式②）に留める。
...
### Step 3: codex で画像生成（背景透過）

1. `work/<slug>/` を作成する（透過PNG の置き場）。
2. Step 2 の設計から**codex 用プロンプト**（Markdown）をスクラッチパッドに書き出す。次を必ず含める。
   - imagegen スキルの「Transparent image requests」に従い、**組み込み `image_gen` に「背景を完全に透明にする」よう指示して生成し、アルファをそのまま保持する**こと
   - **クロマキーで生成しない**こと、`remove_chroma_key.py` を実行しないこと
   - **CLI フォールバック（`scripts/image_gen.py` / `gpt-image-1.5` / `OPENAI_API_KEY`）を使わない**こと
   - 影・グラデーション禁止、白い面は不透明な `#ffffff` で塗ること
3. `codex exec` を**バックグラウンドで**実行し、`work/<slug>/` へ透過 PNG を生成させる。
4. 生成完了を待つ（Monitor で「codex プロセス終了」を待機）。**サブエージェントに任せた場合でも、離脱することがあるのでオーケストレーター側で終了を待ち直す**。
   - コマンド・プロンプトテンプレート・フォールバック（方式②）・リトライ方針は `references/codex-imagegen.md` を参照。

### Step 4: 生成物の検証（**Opus＝本体が目視**）

1. `file work/<slug>/*.png` で寸法（既定 **1536x1024**）を、透過検証コマンドで **RGBA・四隅 alpha 0・透明率 10〜60%** を確認する（`references/codex-imagegen.md`「透過の検証」）。
   - **`mode` が `RGB`（アルファ無し）や透明率がほぼ 0% なら透過に失敗している。** プロンプトの透過指定を強めて再生成し、2回落ちた画像だけ**方式②（クロマキー＋`remove_chroma_key.py`）へ切り替える**。
...
4. 不合格なら Step 3 のプロンプトを修正して**再生成**する（同一章で最大2回まで。3回目で通らなければ所見をユーザーに報告して判断を仰ぐ）。
   - 合否基準は `references/illustration-design.md` の「検収チェック」を参照。

### Step 5: 差し込み指示Markdownの作成と差し込み・ビルド

1. `work/<slug>/差し込み箇所.md` を書き出す（`references/handoff-and-insert.md` の書式）。差し込み対象は **`work/<slug>/` 直下の透過PNG**であり、`_check-dark-*.png`（と方式②を使った場合の `_raw/`）は対象外。
...
| work/ 内のクロマキー原版（方式②のときだけ／中間物） | `_raw/<slug>-<NN>-<topic>.png` | `_raw/markdown-01-overview.png` |
...
- **成果物は背景透過 PNG**。`_check-dark-*.png`（ダーク確認用）と、方式②を使った場合の `_raw/`（クロマキー原版）は中間物であり、`public/` へコピーしない。
...
| `references/codex-imagegen.md` | codex CLI の呼び出し方（`codex exec` コマンド・バックグラウンド実行・完了待ち）／**方式①直接透過生成（既定）と方式②クロマキー（フォールバック）**／透過の検証・ダーク合成／プロンプトテンプレート／スタイル参照画像の選定／リトライとトラブルシュート |
