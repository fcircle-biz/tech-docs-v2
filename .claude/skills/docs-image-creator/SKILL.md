...
> **「第N章の進め方」「章の地図」は作らない。** 節タイトルを並べた図はサイドバー目次と学習目標カードの重複であり、Java 入門ガイドのレビューで「無くても困らない」と判定された。画像は**読者が頭の中で描かなければならない絵**（メモリ上の箱・値の流れ・参照の矢印・Before/After）を代わりに描くために作る。HTML の表・カードで書き直せる図は作らない（`references/illustration-design.md`「図示価値の判定」）。
...
- `<対象章>`: 次のいずれか。いずれの形でも分類パス・slug・章番号を導出する。
  ...
  - 自然言語（例: `docker 第3章`）
- `[枚数]`（任意）: その章に作る画像の枚数の上限。省略時は **「図示価値の判定」を通過した図案の数（既定 1〜3枚・上限4枚・0枚も可）** を本スキルが決める。見出し数で枠を決めて埋めるのではない。指定枚数に届かない場合は無理に作らず、その旨を報告する。
...
2. 章の本文を読み、**読者が想像しにくい仕組み・値の動き・状態変化**を探す（見出しを眺めて1節1枚を割り当てるのではない）。優先順は `references/illustration-design.md`「画像で価値が出る箇所」: ①見えない仕組みの可視化 → ②具体値のトレース → ③同じ対象の Before→After → ④実データ入りの構造対比 → ⑤本文が使っている比喩。
3. 図案ごとに**図示価値の判定（テキストカード判定）**を通す: 「HTML の表・カード・箇条書きで情報を落とさず書き直せるか？」→ 書き直せるなら**不採用**。次は既定で禁止: **「第N章の進め方」「章の地図」**（目次の重複）、用語カードの羅列、良い例／悪い例のリスト、「A に B と C が入っている」だけのボックス図、本文の要約スライド。章導入画像も**既定で作らない**。
4. 棚卸しした既存要素の**要約にすぎない図案は採用しない**。判定を通った図案の数がそのまま枚数（既定 1〜3・上限4・**0枚も可**）。
5. 画像ごとに「図示価値・役割・**具体値**・タイトル・レイアウト・画像内に描く日本語文字列」まで具体的に決める。**本文のコード例の実値・識別子**（`scores` の `80, 90, 75`、`myCar`、`add(3, 5)`）を図に置く。抽象ラベルだけの図はカード羅列に退化する。
6. 論理名・差し込み位置・キャプション・alt に加え、**既存要素の扱い（置換／両方残す）**を決める。差し込み位置は原則**その仕組みを説明している節の中**。
7. **透過前提で構成を決める**。タイトルを含むすべての文字・細線を**不透明なカード／帯の中**に置く設計にする（地に直接置かない）。影・グラデーション・半透明の重ねは使わない。
8. **画像内の文字列を「描ける形」に詰める**（再生成を減らす最重要工程）。
   - 1カード・1帯あたり**14文字以内**。章の節タイトルは**そのまま使わず短縮ラベルを確定**する。14文字の枠は説明文より**具体値**に使う。
   - `―` `—` `〜` `/` を**使わない**（長音や別記号に化ける）。区切りは改行か帯の分割で表す。
   - **アイコンは既定で描かない**（一部だけ空枠が残る事故が起きる）。
   - 設計指針・図示価値の判定表・重複棚卸しの判定表・命名規則・禁止事項・透過規約は `references/illustration-design.md` を参照。
...
2. **各画像を Read で開いて自分の目で確認する**（日本語の誤字・意味不明な綴り・レイアウト崩れ・ロゴ混入・配色ずれ・縁の汚れ）。あわせて**「図示価値の判定」を生成物に再適用**する: 具体値や矢印が落ちて「ラベルの箱を並べただけ」になっていたら、プロンプトではなく図案を直す。
...
- **「第N章の進め方」「章の地図」「○○の全体像」のような節一覧・用語カード羅列は作らない。** 枚数を確保するために低価値の図案で枠を埋めない。判定を通る図案が無ければ0枚で報告する。

## references

| ファイル | 内容 |
|----------|------|
| `references/illustration-design.md` | **図示価値の判定（テキストカード判定・禁止パターン・具体値の必須化）**／枚数の決め方／重複棚卸し／画像内テキスト規約／配色／**透過を前提とした作図規約**／生成物の検収チェック |
