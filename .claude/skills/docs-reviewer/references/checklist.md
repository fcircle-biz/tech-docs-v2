...
**テーブル行の色分け**: `<tr class="bg-primary-50">` は技術色そのままの面で、行1本に敷くとヘッダー行より目立つ。意味の説明が無いまま多用され、1ガイド内で「主役の明示」「小計行」「ゼブラ縞」の3通りに解釈が割れる事故が起きているため、次を確認する。

```bash
# 色分け行を洗い出し、1件ずつ意味を説明できるか確認する
grep -rn '<tr class="bg-primary-50">' <対象ディレクトリ>
```

- [ ] **色分け行に説明できる意味がある**: 各 `<tr class="bg-primary-50">` について「なぜこの行だけ色が付くのか」を一言で言える（例: 合計・小計行）。言えないものは `<tr>` に戻す。
- [ ] **凡例がある**: 色分けを使った表の直後に、色見本付きの凡例が1行ある（`<span class="inline-block w-3 h-3 rounded-sm bg-primary-50 border border-primary-300 ...">` ＋ 説明文）。
- [ ] **ガイド内で意味が1種類**: 同じ色を「合計行」と「注目行」など複数の意味で使い分けていない。
- [ ] **ゼブラ縞に使っていない**: 1行おきの色敷きが無い（行の区切りは `divide-y divide-slate-200` の罫線で表現する）。
- [ ] **太字で足りる強調に色を使っていない**: 単に重要な行なら `font-bold` / `font-semibold` を優先し、色は付けない。

規約の真実源は `astro-system/templates/v1/reference/css-styles.md`「13. テーブル > 行の色分けルール」。
