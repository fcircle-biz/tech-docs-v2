...
`<tr class="bg-primary-50">` による行の色分けは**既定で使わない**。`bg-primary-50` は技術色そのままの面で、行1本に敷くとヘッダー行より目立ち、読者は「なぜこの行だけ色が付いているのか」を探してしまう。実際に、意味の説明が無いまま多用され、1つのガイド内で「主役の明示」「小計行」「ゼブラ縞」の3通りに解釈が割れる事故が起きた。

色分けしてよいのは、次を**すべて**満たすときだけ。

1. その色が何を意味するかを**一言で説明できる**（例: 「上の項目を足し引きした合計・小計行」）。
2. 表の直後に**凡例を1行**添える。
3. **1つのガイド内で意味を1種類に統一**する。同じ色を「合計行」と「注目行」に併用しない。
...
**理由にならないもの**（＝色分けせず `<tr>` のままにする）:

- 「主役の例だから」「なんとなく目立たせたい」… 本文やセル内の文言で説明する。
- **ゼブラ縞** … `divide-y divide-slate-200` の罫線で十分。1行おきに色を敷かない。
- **その行が重要だから** … 色より軽い `font-bold` / `font-semibold` を優先する。太字で足りるなら色は付けない。

真実源は `astro-system/templates/v1/reference/css-styles.md`「13. テーブル > 行の色分けルール」。
